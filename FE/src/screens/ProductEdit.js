...
import "../styles/NewEdit.scss";
import Rating from "../components/Rating";
import { useDispatch, useSelector } from "react-redux";
import { editProduct, productDetail } from "../actions/productActions";
...
const ProductEdit = () => {
  ...
  const [name, setName] = useState(
    productInfo ? productInfo.product.name : product.name
  );
  const [price, setPrice] = useState(
    productInfo ? productInfo.product.price : product.price
  );
  const [description, setDescription] = useState(
    productInfo ? productInfo.product.description : product.description
  );
  let ar = productInfo ? productInfo.product.category : product.category;
  let arr = ar.split(",");
  let arr1 = new Array(4).fill(false);
  if (arr.indexOf("Bottle") !== -1) arr1[0] = true;
  if (arr.indexOf("Cup") !== -1) arr1[1] = true;
  if (arr.indexOf("Metal") !== -1) arr1[2] = true;
  if (arr.indexOf("Plastic") !== -1) arr1[3] = true;
  const [isCheck, setIsCheck] = useState(arr1);
  ...
  const handleEditProduct = (e) => {
    e.preventDefault();
    dispatch(
      editProduct(product._id, name, price, description, category.toString())
    );
    if (disable) {
      ...
      <div className="container">
        <form className="form-container">
          <div className="left-content">
            ...
            <input className="btn-upload" type="button" value="Upload" />
          </div>
